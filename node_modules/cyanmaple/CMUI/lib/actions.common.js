...
var script$1 = {
  ...
  argumentsRole: function argumentsRole(options, args, CURRENT) {
    ...
    if (args.length > 1) {
      var fnList = _.filter(args, _.isFunction);

      options.selectFn = fnList[0];
      options.cancelFn = fnList[1];

      var styleList = _.filter(args, _.isPlainObject);

      if (styleList.length === 1) {
        options.itemStyle = styleList[0];
      }

      var stringList = _.filter(args, function (item) {
      ...
      stringList.forEach(function (item, index) {
        ...
        var style = _.get(styleList, index);
      ...
      var arrArg = _.find(args, _.isArray);
