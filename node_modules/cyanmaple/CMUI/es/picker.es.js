...
function formatWheelData(wheelData, lengthStore) {
  ...
  return _.map(wheelData, function (item) {
...
function formatData(data) {
  ...
  return _.map(data, function (whleeData, index) {
    return formatWheelData(whleeData, _.assign(lengthStore, {
      tempLen: index + 1
    }));
  }).concat(_.fill(Array(lengthStore.maxLen - data.length), [{
    text: '',
    value: undefined
  }]));
}
...
function getInitData(data, value) {
  console.log('innit');
  var pickerData = formatData(_.every(data, _.isArray) ? data : [data]);
  var valueList = _.isArray(value) ? value : [value];
  ...
  var _loop = function _loop(index) {
    ...
    var activeIndex = _.findIndex(item, function (item) {
      return _.isEqual(item.value, valueList[index]);
    });

    activeIndex = ~activeIndex ? activeIndex : 0;
    pickerSelectIndex[index] = activeIndex;

    var chiildrenData = _.get(item, "[".concat(activeIndex, "].children"));
...
var script$1 = {
      ...
      set: function set(value) {
        ...
        if (value && !this.wheels.length) {
          this.$nextTick(function () {
            _.forEach(_this2.pickerData, function (item, index) {
  ...
  methods: {
    ...
    updateData: function updateData(index, active) {
      ...
      var children = _.get(this, "pickerData[".concat(index, "][").concat(active, "].children"));
    ...
    destroyWheels: function destroyWheels() {
      _.forEach(this.wheels, function (item) {
    ...
    getData: function getData() {
      ...
      return _.map(this.wheels, function (item, index) {
        return {
    ...
    setData: function setData(data) {
      ...
      _.delay(function () {
        ...
        _.assign(_this4, getInitData(data, _this4.value));

        _this4.$nextTick(function () {
          _.forEach(_this4.pickerData, function (item, index) {
    ...
    _ok: function _ok() {
      ...
      this.$emit('input', _.isArray(this.value) ? data.map(function (item) {
        return item.value;
...
function dialogInstall(Component) {
  ...
  var defaultsOptions = _.mapValues(props, function (item) {
    return _.isFunction(item.default) ? item.default() : item.default;
  });

  var timeHandle;

  var id = "".concat(name, "-") + _.uniqueId();

  var CURRENT = null;

  var setCurrent = _.once(function (Vue) {
